	...
	for (x = 0, px = 0; x < GOL_X_SIZE; x++) {
		for (y = 0, py = 0; y < GOL_Y_SIZE; y++) {
			...
			if (c) {
				...
				if (c != ON_CELL) {
					fillRect(px + 1, py + 1,
							px + (DISPLAY_WIDTH / GOL_X_SIZE)- 2, py + (DISPLAY_HEIGHT / GOL_Y_SIZE) - 2, drawcolor[color]);

}						}
			py += (DISPLAY_HEIGHT / GOL_Y_SIZE);
		}
		px += (DISPLAY_WIDTH / GOL_X_SIZE);
	}
}
...
void ClearScreenAndDrawGameOfLifeGrid(void) {
	...
	clearDisplay(drawcolor[BG_COLOR]);
	//clear cells
	for (x = 0, px = 0; x < GOL_X_SIZE; x++) {
		for (y = 0, py = 0; y < GOL_Y_SIZE; y++) {
			fillRect(px + 1, py + 1, px - 2 + (DISPLAY_WIDTH / GOL_X_SIZE), py + (DISPLAY_HEIGHT / GOL_Y_SIZE) - 2,
