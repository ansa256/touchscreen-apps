 ...
 * @return uint16_t start x for next character - next x Parameter
 */
uint16_t MI0283QT2::drawText(uint16_t aXStart, uint16_t aYStart, char *aStringPtr, uint8_t aSize, uint16_t aColor,
        uint16_t aBGColor) {
    uint16_t tLength = 0;
    char *tStringPtr = aStringPtr;
    uint16_t tXPos = aXStart;
    while (*tStringPtr != 0) {
        tXPos = drawChar(tXPos, aYStart, (char) *tStringPtr++, aSize, aColor, aBGColor);
        tLength++;
        if (tXPos > DISPLAY_WIDTH) {
            break;
        }
    }
 ...
 */
uint16_t MI0283QT2::drawMLText(uint16_t aPosX, uint16_t aPosY, const char *aStringPtr, uint8_t aTextSize, uint16_t aColor, uint16_t aBGColor) {
    uint16_t x = aPosX, y = aPosY, wlen, llen;
    char c;
    const char *wstart;

    LocalDisplay.fillRect(aPosX, aPosY, DISPLAY_WIDTH - 1, aPosY + (FONT_HEIGHT * aTextSize) - 1, aBGColor);

    llen = (DISPLAY_WIDTH - aPosX) / (FONT_WIDTH * aTextSize); //line length in chars
    wstart = aStringPtr;
    while (*aStringPtr && (y < DISPLAY_HEIGHT - (FONT_HEIGHT * aTextSize))) {
        c = *aStringPtr++;
        if (c == '\n') {
            //new line
            x = aPosX;
            y += (FONT_HEIGHT * aTextSize) + 1;
            LocalDisplay.fillRect(x, y, DISPLAY_WIDTH - 1, y + (FONT_HEIGHT * aTextSize) - 1, aBGColor);
            continue;
        } else if (c == '\r') {
            //skip
            continue;
        ...
        if (c) {
            if ((x + (FONT_WIDTH * aTextSize)) > DISPLAY_WIDTH - 1) {
                //start at new line
                if (c == ' ') {
                    ...
                    if (wlen > llen) {
                        ...
                        LocalDisplay.fillRect(x - (wlen * FONT_WIDTH * aTextSize), y, DISPLAY_WIDTH - 1, (y + (FONT_HEIGHT * aTextSize)),
                                aBGColor);
                        x = aPosX;
                        y += (FONT_HEIGHT * aTextSize) + 1;
                        LocalDisplay.fillRect(x, y, DISPLAY_WIDTH - 1, y + (FONT_HEIGHT * aTextSize) - 1, aBGColor);
