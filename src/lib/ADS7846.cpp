...
void ADS7846::rd_data(int aOversampling) {
	 ...
	 * if DSO is in DMA Mode situation gets worse so 10 ms are only reasonable for factor 14
	 */
	int tDelayValue = ((aOversampling / 8) + 1) * TOUCH_DELAY_AFTER_READ_MILLIS; // formula for worst case
...
static void handleTouchRelease(void) {
	...
	if (!TouchPanel.sTouchReleaseProcessed) {
		TouchPanel.sTouchReleaseProcessed = true;
		...
		if (TouchPanel.mEndTouchCallbackEnabled) {
			TouchPanel.mEndTouchCallback(getMillisSinceBoot() - TouchPanel.ADS7846TouchStartMillis,
...
extern "C" void EXTI1_IRQHandler(void) {
	...
	if (!tLevel) {
		...
		TouchPanel.sTouchReleaseProcessed = false; // reset flag
 ...
 */bool (*
		ADS7846::getEndTouchCallback(void))(uint32_t const, int const, int const) {
