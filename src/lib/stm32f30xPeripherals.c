...
uint16_t sRefintActualValue; // actual value of VREFINT / see *((uint16_t*) 0x1FFFF7BA) = value of VREFINT at 3.3V
...
// get VREFINT_CAL (value of reading calibration input acquired at temperature of 30 °C and VDDA=3.3 V) - see chapter 3.12.2 of datasheet
    ...
    uint32_t tRefActualValue = sRefintActualValue; // 16 -> 32 bit
...
void MI0283QT2_IO_initalize(void) {
    ...
    GPIO_InitStructure.GPIO_Pin = HY32D_CS_PIN | HY32D_DATA_CONTROL_PIN | HY32D_WR_PIN | HY32D_RD_PIN;
...
void AccuCapacity_IO_initalize(void) {
    ...
    GPIO_InitStructure.GPIO_Pin = ACCUCAP_DISCHARGE_1_PIN | ACCUCAP_DISCHARGE_2_PIN | ACCUCAP_CHARGE_1_PIN | ACCUCAP_CHARGE_2_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
...
void ADC2_init(void) {
    ...
    GPIO_InitStructure.GPIO_Pin = ADC2_INPUT1_PIN | ADC2_INPUT2_PIN;
...
#define STM32F3D_SPI1_GPIO_PORT             GPIOA                       /* GPIOA */
 ...
 */
static uint16_t actualPrescaler;
...
#define STM32F3D_PWM_BL_CHANNEL_SET_COMMAND         TIM_SetCompare4
...
void PWM_BL_initalize(void) {
    ...
    /* TIM4 clock enable */
    RCC_APB1PeriphClockCmd(STM32F3D_PWM_BL_TIMER_CLOCK, ENABLE);
    ...
    /* Time base configuration */
    TIM_TimeBaseStructure.TIM_Period = PWM_RESOLUTION_BACKLIGHT - 1;
    TIM_TimeBaseStructure.TIM_Prescaler = PrescalerValue;
    TIM_TimeBaseStructure.TIM_ClockDivision = TIM_CKD_DIV1;
    TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
    TIM_TimeBaseInit(STM32F3D_PWM_BL_TIMER, &TIM_TimeBaseStructure);

    GPIO_InitTypeDef GPIO_InitStructure;

    RCC_AHBPeriphClockCmd(STM32F3D_PWM_BL_GPIO_CLK, ENABLE);
...
void PWM_BL_setOnRatio(uint32_t aOnTimePercent) {
    ...
    STM32F3D_PWM_BL_CHANNEL_SET_COMMAND(STM32F3D_PWM_BL_TIMER, (uint32_t) aOnTimePercent * 256 / 100);
}
...
#define ADC1_INPUT_GPIO_CLK                     RCC_AHBPeriph_GPIOA
...
void ADC_init(void) {
     ...
     * ADC GPIO input pin init
     */
    RCC_AHBPeriphClockCmd(ADC1_INPUT_GPIO_CLK, ENABLE);
// Configure pin A1 + A2 + A3 as analog input
    GPIO_InitStructure.GPIO_Pin = ADC1_INPUT1_PIN | ADC1_INPUT2_PIN | ADC1_INPUT3_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_AN;
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_NOPULL;
    GPIO_Init(ADC1_INPUT_GPIO_PORT, &GPIO_InitStructure);
...
/*
 * read internal Temp sensor by ADC1 channel 16 and convert it
 */
...
float ADC_getTemperature(void) {
    float tActualTemperature = (float) ADC_getTemperatureMilligrades();
...
// Bit 0,1,2 pin, AC range pin
    GPIO_InitStructure.GPIO_Pin = ADC1_ATTENUATOR_0_PIN | ADC1_ATTENUATOR_1_PIN | ADC1_ATTENUATOR_2_PIN | ADC1_AC_RANGE_PIN;
    GPIO_InitStructure.GPIO_Mode = GPIO_Mode_OUT;
    GPIO_InitStructure.GPIO_OType = GPIO_OType_PP;
    GPIO_InitStructure.GPIO_PuPd = GPIO_PuPd_UP;
    GPIO_InitStructure.GPIO_Speed = GPIO_Speed_2MHz;
...
// high priority - to enable interrupting slider handling by systic interrupt
    NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 0;
    NVIC_InitStructure.NVIC_IRQChannelSubPriority = 3;
    NVIC_InitStructure.NVIC_IRQChannelCmd = ENABLE;
    NVIC_Init(&NVIC_InitStructure);
